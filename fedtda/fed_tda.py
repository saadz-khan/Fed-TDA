...
class FedTabularDataSyn:
    ...
    def fit(self, data_list, discrete_columns, client_id=None):
        ...
        :return:
        """
        # check if data in data_list is DataFrame
        if not isinstance(data_list, list):
            raise ValueError('fed_gc need a data list.')
        client_num = len(data_list)
        assert client_num > 1, "data_list must content"
    ...
    def _sample(self, num_rows=1, conditions=None, client_id=None):
        ...
        # one column in original data corresponding one univariate
        # one univariate corresponding one or two columns in samples(but will be transformed into one column)
